document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderBoard() { boardContainer.innerHTML = ''; const project = findCurrentProject(); if (!project) return; project.lists.forEach(list => { const listEl = document.createElement('div'); listEl.className = 'list'; listEl.dataset.listId = list.id; listEl.innerHTML = `<div class="list-header"><h2 class="list-title" contenteditable="true">${list.name}</h2><button class="add-task-btn" data-list-id="${list.id}">+</button></div><div class="task-list"></div>`; const taskListEl = listEl.querySelector('.task-list'); list.tasks.forEach(task => { const taskEl = createTaskElement(task); taskListEl.appendChild(taskEl); }); boardContainer.appendChild(listEl); }); }
    function createTaskElement(task) { const taskEl = document.createElement('div'); taskEl.className = 'task'; taskEl.draggable = true; taskEl.dataset.taskId = task.id; let dueDateHTML = ''; if (task.dueDate) { const date = new Date(task.dueDate); dueDateHTML = `<div class="task-due-date">แจ้งเตือน: ${date.toLocaleString('th-TH')}</div>`; } taskEl.innerHTML = `<h4>${task.title}</h4><p>${task.description}</p>${dueDateHTML}`; return taskEl; }
    addProjectBtn.addEventListener('click', async () => { const projectName = prompt('กรุณาใส่ชื่อโปรเจกต์ใหม่:'); if (projectName) { const newProject = { id: `proj-${Date.now()}`, name: projectName, lists: [{ id: `list-${Date.now()}-1`, name: "Todo", tasks: [] }, { id: `list-${Date.now()}-2`, name: "In Progress", tasks: [] }, { id: `list-${Date.now()}-3`, name: "Complete", tasks: [] }] }; data.projects.push(newProject); data.currentProjectId = newProject.id; await saveData(); render(); } });
    projectSelect.addEventListener('change', async () => { data.currentProjectId = projectSelect.value; await saveData(); renderBoard(); });
    boardContainer.addEventListener('click', (e) => { if (e.target.classList.contains('add-task-btn')) { openTaskModal(null, e.target.dataset.listId); } if (e.target.closest('.task')) { const taskEl = e.target.closest('.task'); openTaskModal(taskEl.dataset.taskId, taskEl.closest('.list').dataset.listId); } });
    ...
    function showLoader() { boardContainer.innerHTML = '<h2>Loading data...</h2>'; }
    function hideLoader() { boardContainer.innerHTML = ''; }
    async function startApp() { await initializeData(); render(); setInterval(checkReminders, 60000); }
    startApp();
});
